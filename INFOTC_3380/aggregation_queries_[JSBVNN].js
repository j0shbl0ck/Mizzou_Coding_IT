...
db.zips.aggregate([
    {$match:{state:"MO"}},
    {$group: {"_id":"$city",TotalPopulation:{$sum:"$pop"}}},
    {$sort:{TotalPopulation: -1}},{$limit: 10}
])
...
db.zips.aggregate([
    {$group: {"_id":"$city",TotalPopulation:{$sum:"$pop"}}},
    {$sort:{TotalPopulation: -1}},{$limit: 15}
])
...
db.stocks.aggregate([
    ...
    {$group: {"_id":"$Name",TotalEarnings:{$sum:"$EBITDA"}}},
